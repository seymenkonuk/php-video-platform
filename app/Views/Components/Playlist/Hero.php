...
<div class="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[22rem_minmax(0,1fr)]">
    <div class="relative aspect-video overflow-hidden bg-slate-100 lg:aspect-auto lg:min-h-72">
        ...
        <span title="<?= $header->videoCount ?> video" class="absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-xl bg-black bg-opacity-70 px-3 py-2 text-sm font-bold text-white">
            <!-- İkon -->
    ...
    <div class="flex flex-col justify-center p-6 sm:p-8">
        ...
        <h1 title="<?= $this->escape($header->title) ?>" class="mt-3 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">
        ...
        <?php endif ?>
        <!-- Kanal Bilgisi -->
        <a href="<?= $this->escape($header->channel->url) ?>" title="<?= $this->escape($header->channel->title) ?>" class="mt-5 flex w-fit items-center gap-3 text-sm font-bold text-slate-700 hover:text-red-600">
            <!-- Kanal Resmi -->
            <img src="<?= $this->escape($header->channel->avatar) ?>" alt="<?= $this->escape($header->channel->title) ?>" class="h-9 w-9 rounded-full object-cover">
            <!-- Kanal İsmi -->
            <span>
                <?= $this->escape($header->channel->title) ?>
            </span>
        </a>
