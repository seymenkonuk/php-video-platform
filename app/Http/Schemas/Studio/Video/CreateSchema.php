    ...
    public function files(): ObjectValidator
    {
        return $this->validator->object()->schema([
            "file" => $this->validator->field()
                ->file()
                ->mimes(ValidationConfig::ALLOWED_VIDEO_MIME_TYPES)
                ->extensions(ValidationConfig::ALLOWED_VIDEO_EXTENSIONS)
                ->min(ValidationConfig::VIDEO_MIN_FILE_SIZE)
                ->max(ValidationConfig::VIDEO_MAX_FILE_SIZE)
                ->required(),
